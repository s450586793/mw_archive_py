...
- 描述区新增 `Boost Me` 卡片样式与更接近目标页的富文本排版规则，评论卡片、标签区、缩略图区和主下载按钮样式同步调整
- 打印配置顶部切换去掉原先不合理的“全部”伪选项，改为直接按实际配置切换；详情页静态资源版本号继续提升，确保浏览器强制刷新到新版

### 2026-04-09（更新 7）
- 批量归档翻页策略新增多参数探测：当作者页或收藏夹页的 `?page=2` 仍返回第一页内容时，会继续尝试 `p / offset / skip / start / cursor` 等常见分页方式
- 批量列表解析日志新增 `new_models` 与 `page_size_hint` 字段，方便区分“第二页没翻到”还是“第二页抓到重复数据”
- `mw-test` 本次部署后已在线实测这条链路，后续作者页/收藏夹页的漏扫会继续按这个分页探测策略迭代

### 2026-04-09（更新 6）
- 模型详情页首屏重新按 MakerWorld 结构重排，标题信息改为横跨双栏，主图与缩略图、打印配置面板、主下载按钮和选中态交互整体对齐目标页布局
- 详情页静态资源版本号已提升，避免浏览器或 DSM 反向代理继续命中旧版 `model.css` / `model.js`
- 本次部署会一并带上前一条“批量归档列表页误扫/漏扫”的解析修复，线上 `mw-test` 不再停留在只发现 `22` 个模型的旧镜像

### 2026-04-09（更新 5）
- 批量归档的列表页解析改为优先提取页面实际模型卡片链接，再回退到最匹配的模型数组，减少把推荐位或浏览历史模型混入结果的问题
- 批量任务日志新增每页解析数量与累计发现数量，便于定位作者页/收藏夹页漏扫或误扫

### 2026-04-09（更新 4）
- 模型库卡片点击行为改为当前页面打开详情，不再默认新开标签页
- 在线详情页重排为更接近 MakerWorld 的双栏详情结构，新增返回入口与评论展示区
- 归档链路新增评论抓取，评论作者、时间、点赞、评分与评论图片会一并写入 `meta.json`

### 2026-04-09（更新 3）
- 模型归档页支持在刷新页面、切换到其他页面后重新回来时自动恢复当前归档任务进度，并将当前标签状态写入 URL
- 安全页新增登录账号密码修改表单，支持在后台直接修改用户名和密码并立即生效
- 登录默认值调整为 `admin / admin`，旧配置若仍是 `admin / change-me` 会自动迁移为 `admin / admin`

### 2026-04-09（更新 2）
- 新增作者上传页与收藏夹模型页批量归档入口，支持把列表页自动展开为模型队列后串行归档
- 归档任务新增进度信息结构，控制台归档页增加实时进度条、任务状态、批量子任务明细与结果汇总展示
- 调整前端归档交互，改为非阻塞轮询和进度卡片反馈，不再依赖归档完成后的阻塞式结果弹窗

### 2026-04-09
- 新增公网可用的单用户登录能力，支持登录页、会话保护、Bearer Token 管理与 API Token 校验
- 补充 Docker / GHCR 构建链路，恢复镜像自动构建，并修复镜像缺少 `itsdangerous` 依赖导致容器启动失败的问题
- 调整控制台导航结构，将“安全”页面并入与“配置信息 / 模型归档 / 缺失记录 / 通知 / 其他功能”同级的标签栏，安全页也同步使用同一组标签导航
