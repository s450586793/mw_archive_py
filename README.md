...
## 当前版本
- `v5.5.0`
- 更新说明见 [doc/logs/v5.5.0_update_log.md](doc/logs/v5.5.0_update_log.md)
- 本次重点：
  - 修复 `BUG-20260304-001`；
  - 新增在线模型详情页编辑功能；
  - 支持归档模型元数据备份与最近备份恢复。
...
### v5.5.0 配置与升级说明
如果你是从旧版本升级到 `v5.5.0`，这次主要新增在线模型详情页编辑能力，建议重点关注详情页交互与备份恢复入口：
- 推荐继续挂载：
  - `./app/data:/app/data`
  - `./app/logs:/app/logs`
...
- 页面入口：
  - 在线模型详情页新增“编辑归档”入口
  - 编辑弹窗支持标题、标签、分类、版本备注与简介富文本编辑
  - 支持 `meta.json` 自动备份与“恢复最近备份”

### v5.5.0 使用建议
- 在线编辑前建议先确认模型详情页内容已加载完成。
- 如需撤销最近修改，可直接使用“恢复最近备份”回滚 `meta.json`。
- 富文本简介当前以轻量工具栏为主，复杂排版建议先在外部整理后再粘贴。
