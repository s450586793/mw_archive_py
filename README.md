...
## 当前版本
- `v5.7.1`
- 更新说明见 [doc/logs/v5.7.0_update_log.md](doc/logs/v5.7.0_update_log.md)
- 本次重点：
  - 新增归档任务队列，支持多模型连续提交并自动串行归档；
  - 新增飞书通知渠道，支持图文推送与自动文字降级；
  - 新增详情页“模型路径”复制能力，兼容跨系统场景；
  - 通知配置页支持公共在线地址、分渠道测试和统一保存。
  - 更新归档插件,支持页面显示是否归档
- 升级提醒：
  - 归档队列当前为内存队列，服务重启后未完成任务不会自动恢复；
  - 飞书图文推送依赖 `app_id + app_secret`，缺失时自动降级为文字推送；
  - 路径按钮为复制方案，不直接拉起系统文件管理器。
